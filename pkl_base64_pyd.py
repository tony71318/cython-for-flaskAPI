...
if platform.system() == 'Windows':
    ...
    for pkl_name in pkl_name_list:
        ...
        process = subprocess.Popen('python3 setup.py ' + pkl_name + ' build_ext --inplace', shell=True, stdout=subprocess.PIPE)
        process.wait()
        if process.returncode == 0:
            print('build pyd success.')
        else:
        ...
        process = subprocess.Popen('mv ' + pkl_name + '.*.so ' + pkl_name + '.so', shell=True, stdout=subprocess.PIPE)
        process.wait()
        if process.returncode == 0:
        ...
        process = subprocess.Popen('mv ' + pkl_name + '.so ' + id_dir, shell=True, stdout=subprocess.PIPE)
        process.wait()
        if process.returncode == 0:
            print('move ' + pkl_name + '.so sucess.')
        else:
            print('something wrong')
